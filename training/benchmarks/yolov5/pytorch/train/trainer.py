...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    with torch_distributed_zero_first(LOCAL_RANK):
        data_dict = data_dict or check_dataset(data, opt_data_dir=opt.data_dir)  # check if None
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP
                ...
                # flagperf
                _mp, _mr, _map50, _map = results[:4]
                training_state.P = _mp
...
def run(opt, training_state=TrainingState(), callbacks=Callbacks()): # flagperf
    # Checks
    if RANK in [-1, 0]:

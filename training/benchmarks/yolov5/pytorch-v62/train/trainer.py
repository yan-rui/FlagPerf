...
def train(hyp, opt, device, callbacks, training_state):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    with torch_distributed_zero_first(LOCAL_RANK):
        data_dict = data_dict or check_dataset(data, opt.data_dir)  # check if None
...
def run(opt, training_state=TrainingState(), callbacks=Callbacks()):
    # Checks
    if RANK in {-1, 0}:
        print_args(vars(opt))
        check_git_status()
    ...
    # Resume
    if opt.resume :  # resume from specified or most recent last.pt
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(opt.evolve):  # generations to evolve
            ...
            # Train mutation
            train(hyp.copy(), opt, device, callbacks, training_state)
            callbacks = Callbacks()
